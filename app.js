...
[Date]
        ...
        function createTrackerCard(caseItem) {
            ...
            // Ordered status flow used by the move ◀ / ▶ buttons. Index 0 is the
            // leftmost column; index N-1 the rightmost.
            const STATUS_ORDER = ['opened', 'prep', 'progress', 'filed', 'appeal', 'won'];
            const curIdx = STATUS_ORDER.indexOf(caseItem.status);
            const canMoveLeft = curIdx > 0;
            const canMoveRight = curIdx >= 0 && curIdx < STATUS_ORDER.length - 1;
            ...
            const safeId = escapeAttr(String(caseItem.id));
            card.innerHTML = `
                ...
                <div class="kanban-card-actions" role="group" aria-label="Card actions">
                    <button type="button" class="kc-btn kc-move" data-action="move-left" data-id="${safeId}"
                            ${canMoveLeft ? '' : 'disabled'}
                            aria-label="Move to previous column"
                            title="Move to previous column">←</button>
                    <button type="button" class="kc-btn kc-move" data-action="move-right" data-id="${safeId}"
                            ${canMoveRight ? '' : 'disabled'}
                            aria-label="Move to next column"
                            title="Move to next column">→</button>
                    <button type="button" class="kc-btn kc-delete" data-action="delete" data-id="${safeId}"
            ...
            // Action buttons — single delegated handler per card. Click handlers on
            // the buttons themselves would also bubble up and trigger dragstart, so
            ...
            card.addEventListener('click', function(e) {
                ...
                if (action === 'move-left') {
                    moveTrackerCase(id, -1);
                } else if (action === 'move-right') {
                    moveTrackerCase(id, +1);
                } else if (action === 'delete') {
                    deleteTrackerCase(id);
                }
            });

            // Prevent the action buttons themselves from initiating a drag — gives
            // a cleaner click experience and avoids accidental drops.
            card.querySelectorAll('.kc-btn').forEach(b => {
        ...
        // Move a case one column left (-1) or right (+1) in the kanban flow.
        // Same persistence + audit + re-render path as the drag-and-drop handler.
        function moveTrackerCase(caseId, direction) {
            try {
                const STATUS_ORDER = ['opened', 'prep', 'progress', 'filed', 'appeal', 'won'];
                const caseItem = window._TRACKER_CASES.find(c => String(c.id) === String(caseId));
                if (!caseItem) return;
                const curIdx = STATUS_ORDER.indexOf(caseItem.status);
                const newIdx = curIdx + direction;
                if (newIdx < 0 || newIdx >= STATUS_ORDER.length) return;
                const oldStatus = caseItem.status;
                caseItem.status = STATUS_ORDER[newIdx];
                Persist.tracker.save();
                AuditLog.log(AuditLog.ACTIONS.MOVE_CASE, { id: caseId, from: oldStatus, to: caseItem.status });
                renderTrackerCases();
            } catch (err) {
                ErrorLog.add(err, 'moveTrackerCase');
                showToast('Could not move card. Please refresh.', 'error');
            }
        }

        // Delete a case from the tracker after confirmation.
        function deleteTrackerCase(caseId) {
            try {
                const caseItem = window._TRACKER_CASES.find(c => String(c.id) === String(caseId));
                if (!caseItem) return;
                const ok = window.confirm(
                    'Delete this case from the tracker?\n\n"' + (caseItem.title || 'Untitled') + '"\n\nThis cannot be undone.'
                );
                if (!ok) return;
                window._TRACKER_CASES = window._TRACKER_CASES.filter(c => String(c.id) !== String(caseId));
                Persist.tracker.save();
                AuditLog.log(AuditLog.ACTIONS.DELETE_CASE, { id: caseId, title: caseItem.title });
                renderTrackerCases();
                showToast('Case deleted', 'success');
            } catch (err) {
                ErrorLog.add(err, 'deleteTrackerCase');
                showToast('Could not delete card. Please refresh.', 'error');
            }
        }
